...
with open('src/main_scratch.js', 'r', encoding='utf-8') as f:
    main_js = f.read()
...
ai_engine_code = r'''
  ...
  const buildRecruiterBriefing = (knowledge) => {
    ...
    if (knowledge.experience && knowledge.experience.length > 0) {
      ...
      text += expSlice.map(e => -  at ).join("\n");
    }
    let hasPortfolio = false;
    let portfolioText = \n\nHis portfolio includes:\n;
    if (knowledge.projects?.length > 0) { portfolioText += -  practical projects\n; hasPortfolio = true; }
    if (knowledge.skills?.length > 0) { portfolioText += -  documented technical skills\n; hasPortfolio = true; }
    if (knowledge.certificates?.length > 0) { portfolioText += -  certifications\n; hasPortfolio = true; }
    if (knowledge.achievements?.length > 0) { portfolioText += -  achievements\n; hasPortfolio = true; }
    if (hasPortfolio) text += portfolioText.trimEnd();

    if (knowledge.projects?.length > 0) {
      text += \n\nSelected projects:\n;
      const projSlice = knowledge.projects.slice(0, 3);
      text += projSlice.map(p => - : ).join("\n");
    }
    if (knowledge.certificates?.length > 0) {
      ...
      text += certsToShow.map(c => -  from ).join("\n");
    }
    if (knowledge.education?.length > 0) {
      text += \n\nEducation:\n;
      text += knowledge.education.map(e => - ,  ()).join("\n");
    }
    return text.trim();
  };
  ...
  const composeResponse = (intent, expanded, context, q) => {
    ...
    if (intent === "identity") {
      ...
      if (topProjects.length > 0 && topProjects[0].title.toLowerCase() !== "ctf") {
        ...
        text = Regarding projects:  is a notable example.  It utilizes .;
      ...
      if (portfolioKnowledge.experience && portfolioKnowledge.experience.length > 0) {
        ...
        text = His experience includes working as a  at . ;
      ...
      if (context.lastEntity) {
        text = Regarding , the portfolio indicates he used relevant tools to solve practical challenges. For example, his projects often involve Python, Linux, and networking utilities.;
      } else {
  ...
  const startChallenge = () => {
    ...
    createMessageBubble("ai").innerHTML = <p><strong></strong></p><p></p>;

    setTimeout(() => {
      createMessageBubble("ai").innerHTML = <p></p>;
      renderQuickActions([
  ...
  const handleChallenge = (query) => {
    ...
    if (q === "hint" || q === "give me a hint" || q === "/help") {
      ...
      createMessageBubble("ai").innerHTML = <p><em>Hint:</em> </p>;
      return true;
    }
    const qData = challengeQuestions[state.challengeIndex];
    ...
    if (isCorrect) {
      ...
      if (state.challengeIndex >= challengeQuestions.length) {
        state.challengeActive = false;
        createMessageBubble("ai").innerHTML = <p><strong></strong></p><p style="font-family: monospace; padding: 8px; background: var(--surface); border-radius: 4px;"></p>;
        defaultActions();
      } else {
        createMessageBubble("ai").innerHTML = <p>Correct. Next: </p>;
      }
    } else {
  ...
  const runScan = () => {
    ...
    msg.innerHTML = <p></p>;
    setTimeout(() => {
      msg.innerHTML = <p><strong></strong></p><p>Found  projects and  certificates.</p>;
      setBusy(false);
      defaultActions();
    }, 800);
  };
  ...
  const displayRecruiterBriefing = () => {
    const briefHtml = <p><strong></strong></p>
      <div style="white-space: pre-wrap; font-size: 0.9em;"></div>
...
if start_idx != -1 and end_idx != -1:
    new_main = main_js[:start_idx] + ai_engine_code + "\n  " + main_js[end_idx:]
