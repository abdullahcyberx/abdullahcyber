const aiLogic = "
  ...
  if (aiToggle && aiAssistant && aiClose && aiForm && aiInput && aiMessages) {
    ...
    const detectIntent = (tokens, rawQuery) => {
      ...
      // Recruiter
      if (q.includes('hire') || q.includes('recruit') || q.includes('candidate') || q.includes('why should i')) scores.recruiter += wStrong;
      ...
      // Projects
      if (q.includes('project') || q.includes('build') || q.includes('develop')) scores.project_match += wMedium;
      if (q.includes('strongest project')) { scores.project_match += wStrong; scores.followup_more += wWeak; }

      // Experience
      if (q.includes('internship') || q.includes('experience')) scores.experience_match += wMedium;

      // Certificates
      if (q.includes('certificate') || q.includes('certification')) scores.certificate_match += wMedium;
    ...
    const clearConversationContext = () => {
      sessionStorage.removeItem("ai_context_v1");
    };
    ...
    const composeResponse = (intent, rawQuery, context, tokens) => {
      ...
      if (intent === 'followup_more') {
        if (rawQuery.includes('what tools') || rawQuery.includes('python')) {
          if (context.type === 'project') {
        ...
        } else if (rawQuery.includes('strongest')) {
           ...
           intent = 'experience_match'; // Redirect to experience
      ...
      if (text) return { text, newType, newId, intent, suggestions }; // Return early if follow-up handled it
      ...
      switch (intent) {
        case 'identity':
          ...
          newType = 'identity';
          suggestions = ["What does he specialize in?", "What experience does he have?", "Show projects"];
          break;
        case 'specialization':
    ...
    const typeWriterEffect = (element, text, speed, onComplete) => {
      ...
      if (prefersReducedMotion || speed <= 0) {
        element.innerHTML = text.replace(/\\*\\*(.*?)\\*\\*/g, '<strong></strong>');
      ...
      // Handle markdown bold parsing via chunks
      const tokens = [];
      const regex = /\\*\\*(.*?)\\*\\*/g;
      let lastIdx = 0;
      let match;
      while ((match = regex.exec(text)) !== null) {
        ...
        tokens.push({ type: 'bold', val: match[1] });
      ...
      const step = (time) => {
        ...
        if (time - lastTime > speed) {
          ...
          if (token.type === 'bold') {
            currentElement.textContent += token.val[currentTokenCharIdx];
          } else {
            currentElement.textContent += token.val[currentTokenCharIdx];
          }
          ...
          if (currentTokenCharIdx >= token.val.length) {
            currentTokenIdx++;
    ...
    const processQuery = (rawQuery) => {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const animation = typeWriterEffect(bubble, response.text, 15, () => {
...
Set-Content ai_engine.js -Value 
