...
try {
  ...
  experience.forEach((e) => {
    ...
    expIds.add(e.id);
    if (typeof e.displayOrder !== "number")
      errors.push(`Experience: invalid displayOrder for ${e.id}`);
  });
  ...
  projects.forEach((p) => {
    ...
    if (p.repositoryUrl && !isValidUrl(p.repositoryUrl))
      errors.push(`Project ${p.id}: invalid repository URL`);
    if (p.liveUrl && !isValidUrl(p.liveUrl))
      errors.push(`Project ${p.id}: invalid live URL`);
  });
  ...
  skills.forEach((s) => {
    ...
    if (s.skillKey) {
      if (skillKeys.has(s.skillKey))
        errors.push(`Skill: duplicate skill key ${s.skillKey}`);
      skillKeys.add(s.skillKey);
    }

    if (
      typeof s.proficiencyValue !== "number" ||
      s.proficiencyValue < 0 ||
      s.proficiencyValue > 100
    ) {
  ...
  certificates.forEach((c) => {
    ...
    if (certSlugs.has(c.slug))
      errors.push(`Certificate: duplicate slug ${c.slug}`);
    certSlugs.add(c.slug);

    if (c.verificationUrl && !isValidUrl(c.verificationUrl))
      errors.push(`Certificate ${c.id}: invalid URL`);
    if (c.certificateFile && !hasSafeExtension(c.certificateFile))
      errors.push(`Certificate ${c.id}: unsafe file extension`);
    if (typeof c.featured !== "boolean")
      errors.push(`Certificate ${c.id}: featured must be boolean`);
  });
